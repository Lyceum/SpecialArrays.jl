# Modified from https://github.com/JuliaArrays/ElasticArrays.jl
# Copyright (c) 2017: Oliver Schulz oschulz@mpp.mpg.de
# Copyright (c) 2020: Colin Summers colinxs@cs.washington.edu
...
@inline function Base.size(A::ElasticArray, d)
    if d > ndims(A)
        return 1
    elseif d == ndims(A)
        return div(length(eachindex(A.data)), A.kernel_length)
    else
        A.kernel_size[d]
    end
end
...
@propagate_inbounds Base.setindex!(A::ElasticArray, x, i::Int) = (setindex!(A.data, x, i); return A)
...
function Base.similar(
    bc::Broadcast.Broadcasted{Broadcast.ArrayStyle{ElasticArray}},
    ::Type{ElType},
) where {ElType}
